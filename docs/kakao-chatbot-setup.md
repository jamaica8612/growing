...
여기까지만 설정해도 학생 연결과 챗봇 조회 기능을 사용할 수 있다.

학부모가 카카오 채널을 차단하거나 다시 추가했을 때 연결 상태를 자동으로 동기화하려면 아래 값도 선택적으로 입력한다.

- 카카오 디벨로퍼스 앱의 Primary Admin 키
- 카카오톡 채널 공개 ID (`channel_public_id`, 예: `_AbCdE`)
- 카카오톡 채널 UUID (`channel_uuid`, 예: `@growing`)

Primary Admin 키는 카카오 디벨로퍼스의 `앱 > Admin 키`에서 확인한다. 앱에 저장할 때 SHA-256 해시로 바뀌며 평문은 다시 표시되지 않는다. 이 세 값은 학생 연결 자체에는 필요하지 않다.
...
- 관리자가 코드를 발급하거나 학부모에게 따로 전달할 필요가 없다.
- 학부모가 `학생 연결`을 선택하고 학생 이름과 학원에 등록된 학부모 휴대폰 전체 번호를 한 메시지에 입력한다.
- 이름과 휴대폰 번호가 재원생 등록정보와 정확히 일치하면 짧은 개인정보 안내와 `동의하고 연결` 버튼을 보여준다.
- 동의하면 바로 연결된다. 입력한 이름·휴대폰 번호는 학원 등록정보 대조에만 사용하고 그로잉의 연결정보와 운영로그에는 별도로 저장하지 않는다.
- 반복적인 일치 실패는 시도 횟수 제한으로 차단한다.
...
1. 카카오 관리 화면에서 Skill secret을 저장
2. 카카오 챗봇 관리자에 Skill URL과 `x-kakao-skill-secret` 헤더를 등록
3. 연결 전 상태에서 `오늘 출결` 입력 -> 학생 연결 안내가 나오는지 확인
4. 학부모가 `학생 연결`을 선택하고 재원생 이름과 등록된 학부모 휴대폰 전체 번호를 한 메시지에 입력
5. 짧은 개인정보 안내를 확인한 뒤 `동의하고 연결`을 눌러 연결 성공 확인
6. 틀린 이름·휴대폰 번호를 반복 입력했을 때 추가 시도가 잠시 제한되는지 확인
...
채널 차단·재추가 자동 동기화를 사용할 경우에만 Primary Admin 키와 채널 Public ID·UUID를 저장하고 Event URL 동작을 추가로 테스트한다.

## 5. 운영 원칙

- 학부모용 챗봇은 처음에는 정해진 업무 메뉴만 제공한다.
- 학생 개인정보를 외부 생성형 AI로 보내는 자유 답변은 사용하지 않는다.
- 학생 연결은 재원생 이름과 등록된 학부모 휴대폰 전체 번호 일치, 명시적 동의, 원자적 시도 횟수 제한으로 검증한다.
- 연결 확인을 위해 입력한 이름·휴대폰 번호는 등록정보 대조에만 사용하고 그로잉의 연결정보와 운영로그에는 별도로 저장하지 않는다.
- 연결 동의 전 개인정보 안내를 표시하며 상세 고지는 `https://growing.jamaifamily.duckdns.org/privacy.html`에서 제공한다.
