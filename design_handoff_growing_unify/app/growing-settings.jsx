...
const DAILY_TEMPLATE_DEFAULT = `[그로잉영어] {학생명} 오늘의 수업 안내 🌱
...
function GrowingSettings({ variant = 'desktop' }) {
  ...
  const [dailyTpl, setDailyTpl] = useState(DAILY_TEMPLATE_DEFAULT);
  ...
  return (
    <div className={`gd-root ${mobile ? 'gd-mobile' : ''}`}>
      ...
      {/* 종합 알림장 템플릿 */}
      <section className="gd-card set-accent-warn">
        <h3 className="set-h"><Icon name="msg" size={17} /> 종합 알림장 템플릿</h3>
        <p className="set-p">알림장은 <b>종합 알림장 한 가지</b>만 사용합니다. 아래 기본 문구를 고치면 발송 시 <b>{'{학생명} {등원시간} {하원시간} {숙제상태} {보강}'}</b> 토큰이 실제 값으로 치환됩니다. (등·하원·숙제·보강 개별 템플릿은 더 이상 쓰지 않아요.)</p>
        <textarea className="set-memo" value={dailyTpl} onChange={e => setDailyTpl(e.target.value)} style={{ minHeight: '11rem' }} />
        <div className="set-memo-foot">
          <button className="set-tpl-reset" disabled={dailyTpl === DAILY_TEMPLATE_DEFAULT} onClick={() => { setDailyTpl(DAILY_TEMPLATE_DEFAULT); flash('기본 문구로 되돌렸어요'); }}><Icon name="refresh" size={11} /> 기본값</button>
          <button className="pay-btn primary" onClick={() => flash('종합 알림장 템플릿을 저장했어요')}>템플릿 저장</button>
        </div>
      </section>
