          ...
          <section className="gd-card at-class" key={cls.id}>
            ...
            {!mobile && (
              <div className="at-table-head">
                <span>학생</span><span>등 · 하원</span><span>출결</span><span>숙제</span><span>학부모 알림</span><span>비고 메모</span>
              </div>
            )}
                ...
                return (
                  <div className={`at-row ${r.checkInTime || r.status === 'absent' || r.status === 'makeup' ? 'done' : ''}`} key={id}>
                    ...
                    <div className="at-cell at-actions">
                      <button className={`at-act ${copied === `${id}|${cls.id}` ? 'done' : ''}`} onClick={() => copyKakao(id, cls.id, st.name)} title="카톡 메시지 복사">
                        {copied === `${id}|${cls.id}` ? <Icon name="check" size={13} /> : <Icon name="msg" size={13} />} 카톡</button>
                      <button className="at-act primary" disabled={!st.parentContact} title={st.parentContact || '연락처 없음'}><Icon name="send" size={13} /> 문자</button>
                    </div>
